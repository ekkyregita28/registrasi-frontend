...
import { GETMEMBERBYADMINREGIS, GETUSER, GETPOSITION, GETPROVINSIBYID, GETDAERAHBYID } from '../utils/url'
...
const VerifikasiRegistrasi = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const memberPagRes = await (await axios.get(GETMEMBERBYADMINREGIS + "/" + localStorage.getItem("user"))).data
        ...
        <Col>
          <Card small className="mb-4">
            <CardHeader className="border-bottom">
              <h5 className="m-0">Pengajuan Registrasi</h5>
            </CardHeader>
                ...
                <tbody>
                  {
                    ...
                    data.map(item => {
                      return item.status === 'pengajuan pendaftaran' ?
                        <tr>
                          <td>{number++}</td>
...
export default VerifikasiRegistrasi;
